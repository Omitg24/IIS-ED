 ...
 * @version 05/11/2021
 */
public class BSTreePruebas <T extends Comparable<T>>{
	/**
	 * Atributo raiz
	 */
	private BSTNodePruebas<T> raiz;	
	/**
	 * getter del nodo raiz del arbol
	 * 
	 * @return raiz, raiz del nodo
	 */
	protected BSTNodePruebas<T> getRoot() {
		return raiz;
	}	
	/**
	 ...
	 */
	protected void setRoot(BSTNodePruebas<T> raiz) {
		this.raiz = raiz;
	}	
	/**
	 ...
	 */
	private BSTNodePruebas<T> recursiveAddNode(T info, BSTNodePruebas<T> raiz) {
		if (raiz==null) {
			return new BSTNodePruebas<T>(info);
	...
	*/
	public BSTNodePruebas<T> searchNode(T info) {
		if (info==null) {
			return null;
		}		
	 ...
	 * @return true o false
	 */
	private BSTNodePruebas<T> recursiveSearch(T info, BSTNodePruebas<T> raiz) {
		if (raiz == null) {
			return null;
		}
	 ...
	 */
	private String recursivePreOrder(BSTNodePruebas<T> raiz, String result) {
		if (raiz==null) {
			return "";
		}
	 ...
	 * @return result, recorrido
	 */
	private String recursivePostOrder(BSTNodePruebas<T> raiz, String result) {
		if (raiz==null) {
			return "";
		}		
	 ...
	 * @return result, recorrido
	 */
	private String recursiveInOrder(BSTNodePruebas<T> raiz, String result) {		
		if (raiz==null) {
			return "";
		}
	...
	*/
	 ...
	 */
	private BSTNodePruebas<T> recursiveRemoveNode(T info, BSTNodePruebas<T> raiz) {
		if (raiz==null) {
	 ...
	 * @return nodo máximo
	 */
	protected T maxNode(BSTNodePruebas<T> raiz) {
		if (raiz==null) {
			return null;
		}
	 ...
	 * @return nodo máximo
	 */
	private T recursiveMaxNode(BSTNodePruebas<T> raiz) {
	 ...
	 */
	protected String tumbarArbol(BSTNodePruebas<T> p, int esp) {
	...
	/**
	 * Método que comprueba si un arbol es APE
	 * 
	 * @return true o false
	 */
	public boolean isAPE() {
		...
		int nodosIzda = recursiveIsAPE(getRoot().getLeft());
		int nodosDcha= recursiveIsAPE(getRoot().getRight());
		return (Math.abs(nodosIzda - nodosDcha) <= 1);
	}
	/**
	 * Método recursivo que comprueba si un arbol es APE
	 * @param root
	 * @return
	 */
	private int recursiveIsAPE(BSTNodePruebas<T> root) {
		if (root==null) {
			return 0;
		}		
		return 1 + recursiveIsAPE(root.getLeft()) + recursiveIsAPE(root.getRight());
	}
	...
	/**
	 * Método recursivo que calcula la altura del arbol
	 * @param root, raiz
	 * @return altura
	 */
	private int getHeight(BSTNodePruebas<T> root) {
		if (root==null) {
			return 0;
		}
		int altIzda = getHeight(root.getLeft());
		int altDcha = getHeight(root.getRight());
		return 1 + Math.max(altIzda, altDcha);
	}
	
	/**
	 * Método que devuelve si el arbol es AVL o no
	 * 
	 * @return true o false
	 */
	public boolean isAVL() {
		...
		int altIzda = getHeight(getRoot().getLeft());
		int altDcha = getHeight(getRoot().getRight());
		return (Math.abs(altIzda - altDcha) <= 1); 
//		return recursiveIsAVL(getRoot());
	}
//	private boolean recursiveIsAVL(BSTNodePruebas<T> root) {
//		if (root!=null) {
//			boolean avl = true; 
//			if (root.getLeft()!=null) {
//				avl = avl && recursiveIsAVL(root.getLeft());
//			}
//			if (root.getRight()!=null) {
//				avl = avl && recursiveIsAVL(root.getRight());
//			} 
//			
//			return avl;
//		} else {
...
//			int altDcha = getHeight(root.getRight());
//			
//			int diferencia = altDcha - altIzda;

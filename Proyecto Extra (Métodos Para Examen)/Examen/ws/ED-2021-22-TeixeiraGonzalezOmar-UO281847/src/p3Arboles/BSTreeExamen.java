...
public class BSTreeExamen <T extends Comparable<T>>{
	...
	*/
	...
	/**
	 * Método que devuelve si un arbol es perfectamente balanceado
	 * 
	 * @return true o false
	 */
	public boolean isPerfectlyBalanced() {
		return recursiveIsAPE(raiz);
	}
	
	/**
	 * Método recursivo que comprueba si es un APE
	 * @param node
	 * @return
	 */
	private boolean recursiveIsAPE(BSTNode<T> node){
		if (node==null) {
			...
			if (isAPENode(node.getLeft()) && isAPENode(node.getRight())) {
				recursiveIsAPE(node.getLeft());
				recursiveIsAPE(node.getRight());
				return true;
			}
			return false;
		}
	...
	private boolean isAPENode(BSTNode<T> node) {
...
//	/**
//	 * Método recursivo (no usado) para calcular el numero de nodos
//	 * 
//	 * @return numNodos
//	 */
...
//	}
	...
	/**
	 * Método auxiliar (como el search) pero que devuelve un BSTNode.
	 * Hecho para probar el método numNodes por separado
	 * 
	...
	/**
	 * Método recursivo de busqueda del nodo (para el Find auxiliar)
	 * 
	 * @param info, info del nodo
	 * @param raiz, raiz del nodo
	 * @return BSTNode

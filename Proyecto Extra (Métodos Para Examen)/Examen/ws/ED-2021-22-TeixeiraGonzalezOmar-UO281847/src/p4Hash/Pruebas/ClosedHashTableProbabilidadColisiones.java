 ...
 * @param <T>, genérico
 */
public class ClosedHashTableProbabilidadColisiones<T> extends AbstractHashPruebas<T> {
// IMPORTANTE
//	No cambiar el nombre ni visibilidad de los atributos protected
	...
	public ClosedHashTableProbabilidadColisiones(int tam, int expl) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ClosedHashTableProbabilidadColisiones(int tam, double fcUP, double fcDOWN, int expl) { // Para la segunda clase
	//	Completar lo que falta...
		hashSize = nextPrimeNumber(tam);// Establece un tamaño valido si tam no es primo		
		associativeArray = (HashNodePruebas<T>[]) new HashNodePruebas[hashSize]; // Crea el array de HashNode's
		for (int i=0; i<hashSize; i++) {
	...
	//	Completar lo que falta...
		if (getLF() > fcUP || getProbabilidadColision() >  0.7) {			
			this.a=0;
			this.b=0;
	...
	/**
	 * Método que calcula la probabilidad de la colision
	 * @return probabilidad
	 */
	protected double getProbabilidadColision() {
		if (a==0 || b ==0) {
			return 0;
		}
		double probabilidad = a/b; 
		return probabilidad;
	}

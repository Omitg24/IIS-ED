 ...
 * @param <T>, genérico
 */
public class ClosedHashTableDeletedRate<T> extends AbstractHashPruebas<T> {
// IMPORTANTE
//	No cambiar el nombre ni visibilidad de los atributos protected
	...
	public ClosedHashTableDeletedRate(int tam, int expl) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ClosedHashTableDeletedRate(int tam, double fcUP, double fcDOWN, int expl) { // Para la segunda clase
	//	Completar lo que falta...
		hashSize = nextPrimeNumber(tam);// Establece un tamaño valido si tam no es primo		
		associativeArray = (HashNodePruebas<T>[]) new HashNodePruebas[hashSize]; // Crea el array de HashNode's
		for (int i=0; i<hashSize; i++) {
	...
	/**
	 * Redispersion borrados
	 * @return true o false
	 */
	@SuppressWarnings("unchecked")
	protected boolean redispersionDeletedRate() {
	...
	{
		float nCeldas = associativeArray.length;
		float celdasBorradas = 0;
		for(int i = 0; i < associativeArray.length; i++)
		{
			if(associativeArray[i].getStatus() == HashNodePruebas.BORRADO)
			{
				celdasBorradas++;
			}
		}
		
		return celdasBorradas / nCeldas;
	}
}

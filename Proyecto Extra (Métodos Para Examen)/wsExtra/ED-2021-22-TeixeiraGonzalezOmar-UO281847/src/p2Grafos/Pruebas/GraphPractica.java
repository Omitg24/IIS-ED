...
public class GraphPractica <T>{   
    ...
    */   
	...
	public GraphPractica (int tam, T initialNodes[], boolean[][] initialEdges, double [][] initialWeights) {
    ...
    public GraphPractica (int tam, T initialNodes[], boolean[][] initialEdges, double [][] initialWeights, double [][] initialAfloyd, int [][] initialPfloyd) { 
    ...
    /**
     * Método que comprueba que un grafo es conexo
     * 
     * @return true o false
     */
    ...
    /**
     * Método que cuenta el numero de nodos sumidero
     * 
     * @return counterSumideros, counterSumideros
     */
    public int contarSumideros() {
    	int counterSumideros = 0;
    	int counterPosiciones = 0;
    	boolean unoAlMenos = false;
    	for (int i=0; i<numNodes; i++) {
    		for (int j=0; j<numNodes; j++) {
    			if (!edges[i][j]) {
    				counterPosiciones++;
    			...
    			if (counterPosiciones==numNodes&& unoAlMenos) {
    				counterSumideros++;
    			}
    		}
    		unoAlMenos=false;
    	...
    	return counterSumideros;
    }    
    /**
     * Método que cuenta el número de nodos fuente
     * 
     * @return counterFuentes, counterFuentes
     */
    public int contarFuentes() {
    	int counterFuentes = 0;
    	int counterPosiciones = 0;
    	boolean unoAlMenos=false;
    	for (int i=0; i<numNodes;i++) {
    		for (int j=0; j<numNodes;j++) {
    	...
    	return counterFuentes;
    }    
    /**
     * Método que cuenta el número de nodos inaccesibles
     * 
     * @return counterInaccesibles
     */
    public int contarInaccesibles() {
//    	int counterInaccesibles=0;
//        int counterPosiciones=0;
//        for(int i=0;i<numNodes;i++) {
//            for(int j=0;j<numNodes;j++) {
//                if(!edges[j][i]) {
...
//        }
//        return counterInaccesibles;
    	return contarAislados() + contarFuentes();
    }    
    /**
     * Método que cuenta el número de nodos aislados
     * 
     * @return counterAislados, counterAislados
     */
    public int contarAislados() {
    	int counterAislados=0;
        int counterPosiciones=0;
        for(int i=0;i<numNodes;i++) {
            for(int j=0;j<numNodes;j++) {
        ...
        return counterAislados;
    }    
    /**
     * Método que calcula la reciprocidad de las aristas
     * 
     * @return reciprocity, reciprocidad
     */
    public double reciprocity() {
    	double edges = 0;
    	double reciprocityEdges = 0;
    	...
    	return (reciprocityEdges / edges);
    }
    /**
     * Método que cuenta el número de ciclos, debe invocar al método de floyd que permite que haya ciclos
     * 
     * @return true o false
     */
    public boolean contieneCiclos() { 
    	for (int i=0; i<numNodes; i++) {
    		for (int j=0; j<numNodes;j++) {
    			if (i!=j && getAFloyd()[i][j]!=infinite && getAFloyd()[j][i]!=infinite) {
    				return true;
    			}
    		}
    	}
    ...
    /**
     * Método que obtiene el centro de un grafo
     * 
     * @return centro, centro
     */
    public T obtenerCentro() {
    	floyd();
    	double[] vector = new double[numNodes];    	
    	for (int i=0; i<numNodes; i++) {
    		vector[i] = getMax(i);
    	}
    ...
    /**
     * Método que obtiene el valor minimo de un vector
     * 
     * @param vector, vector
     * @return min, min
     */
    private int getMin(double[] vector) {
    	double min = infinite;
    	int position = -1;
    ...
    /**
     * Método que devuelve el valor máximo en la columna
     * 
     * @param i, i
     * @return max, max
     */
	...
	/** 
	 * Devuelve el numero de nodos n en los cuales es posible volver a si mismos 
	 * con un coste no mayor al dado
	 * 
	 * @param source, nodo origen
	 * @param cost, coste
	 * @return numero de nodos que permiten eso
	 */
	public int getNumberOfReturnNodesWithinCost(T source, double cost) {
		...
		int n = 0;
		for (int i = 0; i < numNodes; i++) {
			if (aFloyd[index][i] + aFloyd[i][index]<= cost) {
				n++;
			}
		}
		return n;	
	...
	/**
	 * Método que devuelve los nodos accesibles desde el nodo
	 * 
	 * @param node, nodo
	 * @return vector, vector
	 */
	public String accesiblesFromNode(T node) {
		int index = getNode(node);
		int[] vector= new int[numNodes];
		if (node==null || index==-1) {
			return null;
		}
		if (getPFloyd()==null || getAFloyd()==null) {
			floyd();
		}
		for (int j=0; j<numNodes;j++) {
			if (existeCamino(index,j)) {
				vector[j]=j;
			}
		}
	...
	/**
	 * Método que devuelve los nodos inaccesibles desde el nodo
	 * 
	 * @param node, nodo
	 * @return vector, vector
	 */
	public String inaccesiblesFromNode(T node) {
		int index = getNode(node);
		int[] vector= new int[numNodes];
		if (node==null || index==-1) {
			return null;
	...
	/**
	 * Método que imprime una cadena del vector
	 * 
	 * @param vector, vector
	...
	/**
	 * Método que devuelve el grado de salida de un nodo
	 * 
	 * @param node, nodo
	...
	/**
	 * Método que devuelve el grado de entrada de un nodo
	 * 
	 * @param node, nodo
	 * @return counter, contador
	 */

 ...
 * @param <T>, genérico
 */
public class ClosedHashTableColisionesTotales<T> extends AbstractHashPruebas<T> {
// IMPORTANTE
//	No cambiar el nombre ni visibilidad de los atributos protected
	...
	public ClosedHashTableColisionesTotales(int tam, int expl) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ClosedHashTableColisionesTotales(int tam, double fcUP, double fcDOWN, int expl) { // Para la segunda clase
	//	Completar lo que falta...
		hashSize = nextPrimeNumber(tam);// Establece un tamaño valido si tam no es primo		
		associativeArray = (HashNodePruebas<T>[]) new HashNodePruebas[hashSize]; // Crea el array de HashNode's
		for (int i=0; i<hashSize; i++) {
	...
	//	Completar lo que falta...
		if (elem==null) {
			return -2;
		} else if (findWithoutColisiones(elem) == null || numElems == 0) {
			return -1;
		} else {
			int pos = fHash(elem);
			int iteraccion = 1;
	...
	/**
	 * Método find sin colisiones
	 * 
	 * @param elem, elemento a buscar
	 * @return elem o null
	 */
	public T findWithoutColisiones(T elem) {
			if (elem==null) {
				return null;
			} else {
				int pos = fHash(elem);
